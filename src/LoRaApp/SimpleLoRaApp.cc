...
void SimpleLoRaApp::detectForestFire()
{
    if (!par("enableFireDetection").boolValue()) {
        EV << "Forest fire detection is disabled." << endl;
        return;
    }

    double temperatureThreshold = par("criticalTemperature").doubleValue(); // Threshold in Celsius
    double humidityThreshold = par("criticalHumidity").doubleValue();       // Threshold as a fraction

    double currentTemperature = lastSentMeasurement; // Assume measurement represents temperature
    double currentHumidity = uniform(10, 50) / 100.0; // Simulated humidity (fraction)

    EV << "Current Temperature: " << currentTemperature << "C" << endl;
    EV << "Current Humidity: " << currentHumidity * 100 << "%" << endl;

    if (currentTemperature >= temperatureThreshold && currentHumidity <= humidityThreshold) {
        EV << "Forest fire detected! Sending alert..." << endl;

        auto alertPacket = new Packet("ForestFireAlert");
        auto alertPayload = makeShared<LoRaAppPacket>();
        alertPayload->setChunkLength(B(20)); // Payload size
        alertPayload->setSampleMeasurement(currentTemperature); // Include temperature
        alertPayload->setMsgType(1001); // Custom message type

        auto loraTag = alertPacket->addTagIfAbsent<LoRaTag>();
        loraTag->setBandwidth(getBW());
        loraTag->setCenterFrequency(getCF());
        loraTag->setSpreadFactor(getSF());
        loraTag->setCodeRendundance(getCR());
        ...
        alertPacket->insertAtBack(alertPayload);
        send(alertPacket, "socketOut");
    } else {
        EV << "No forest fire detected." << endl;
    }
}
